<x-app-layout>
                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form method="POST" action="{{ route('tutor.profile.update') }}" class="space-y-6">
                        ...
                        <div>
                            ...
                            <x-text-input id="hourly_rate" name="hourly_rate" type="number" step="0.01" class="mt-1 block w-full" :value="old('hourly_rate', $tutorProfile->hourly_rate)" required />
                            <x-input-error class="mt-2" :messages="$errors->get('hourly_rate')" />
                        </div>

                        <div>
                            <x-input-label for="experience_years" :value="__('Years of Experience')" />
                            <x-text-input id="experience_years" name="experience_years" type="number" class="mt-1 block w-full" :value="old('experience_years', $tutorProfile->experience_years)" required />
                            <x-input-error class="mt-2" :messages="$errors->get('experience_years')" />
                        </div>

                        <div>
                            <x-input-label for="bio" :value="__('Bio')" />
                            <textarea id="bio" name="bio" class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm" rows="4" required>{{ old('bio', $tutorProfile->bio) }}</textarea>
                        ...
                        <div>
                            ...
                            <select id="subjects" name="subjects[]" class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm" multiple required>
                                @foreach($subjects as $subject)
                                    <option value="{{ $subject->id }}" {{ in_array($subject->id, $tutorProfile->subjects->pluck('id')->toArray()) ? 'selected' : '' }}>
                                        {{ $subject->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="education-section">
                            ...
                            <div id="education-entries">
                                @foreach($tutorProfile->education as $index => $education)
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            let educationCount = {{ count($tutorProfile->education) }};
